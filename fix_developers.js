...
// Find where the corruption starts (around the state declarations)
const corruptStart = content.indexOf("const [");
if (corruptStart > 0) {
  // Find the first import statement
  const importEnd = content.indexOf('function Developers');
  if (importEnd > 0) {
    // Keep only up to the function declaration
    const goodPart = content.substring(0, importEnd);
    console.log('Found good part, length:', goodPart.length);
    
    // Now we need to rebuild the function body
    // For now, let's just write a minimal working version
    const minimalContent = `import React, { useState, useEffect } from 'react';
...
`;
  ...
  fs.writeFileSync('src/components/Developers.jsx', minimalContent, 'utf8');
  console.log('File written successfully! Length:', minimalContent.length);
} else {
